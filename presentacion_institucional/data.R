...
boletin_y_procesamiento = 1600000
...
df <- data.frame( año = c(2017,2018,2019,
                  ...
                  concepto = c("gasto_personal","gasto_personal", "gasto_personal", # prespuesto personal
                               "ingreso_capital", "ingreso_capital", "ingreso_capital", # desarrollo software
                               "aportes_no_rbbles")) %>% # aporte MINJUS
  mutate(concepto = factor(concepto, levels = c("gasto_personal", "ingreso_capital", "aportes_no_rbbles"), ordered = T)) %>% 
  mutate(monto = round(monto/1000000, digits = 3)) %>% 
  filter(monto != 0) 

Ingresos_por_Aportes_NRbles <- 2900000/1000000

ingresos <- str_c("$", round(sum(df$monto[df$concepto == "ingreso_capital"], Ingresos_por_Aportes_NRbles),
                             digits = 2), " millones")
